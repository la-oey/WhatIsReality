...
s.bias <- sender %>%
  ...
  mutate(bias = round(bias.ksay, 1),
         ...
         bias = paste0("Delta[report - truth] == ", bias),
         cost = paste(cost, "cost"),
         goal = paste(goal, "goal"))
...
r.bias <- receiver %>%
  ...
  mutate(bias = round(bias.kest, 1),
         bias = ifelse(bias > 0, paste0("+",bias), as.character(bias)),
         bias = paste0("Delta[inferred - report] == ", bias),
         cost = paste(cost, "cost"),
         goal = paste(goal, "goal"))
...
ggsave("img/final/player_hist.png", width=7.5, height=5)
# ggsave("img/player_hist2.png", width=7.5, height=6)
```
...
summary(lmer(bias.ksay ~ goal + (1 | subjID), data=sender.stat))
summary(lmer(bias.ksay ~ goal + goal:cost + (1 | subjID), data=sender.stat))
summary(lmer(bias.ksay ~ goal * cost + (1 | subjID), data=sender.stat))
summary(lm(bias.ksay ~ goal + goal:cost, data=sender.stat))

summary(lmer(ksay ~ k + goal + (1 | subjID), data=sender.stat))
summary(lmer(ksay ~ k + goal + goal:cost + (1 | subjID), data=sender.stat))
summary(lmer(ksay ~ k + goal * cost + (1 | subjID), data=sender.stat)) #this will do
summary(lmer(ksay ~ k + goal * cost + (1 | subjID), data=sender.stat))$coef

m.full <- lmer(ksay ~ k * goal * cost + (1 | subjID), data=sender.stat)
summary(m.full)
m.noInterKGoal <- lmer(ksay ~ k + goal * cost + k:cost + k:goal + (1 | subjID), data=sender.stat)
...
contrasts(receiver.stat$goal) <- c(1,-1)
summary(lmer(bias.ksay ~ goal + (1 | subjID), data=receiver.stat))
...
ggplot() +
  ...
  geom_text(data=data.frame(goal="overestimate goal"), 
            aes(x=-70, y=120, label="Sender"), parse=TRUE,
            family="Optima", size=textsize) +
  geom_text(data=data.frame(goal="overestimate goal"), 
            aes(x=-70, y=-120, label="Judge"), parse=TRUE, colour="white",
            family="Optima", size=textsize) +
  geom_text(data=s.bias, aes(x=47, y=600, label=bias), 
            parse=TRUE, family="Optima", size=textsize) +
  geom_text(data=r.bias, aes(x=47, y=-600, label=bias), 
            parse=TRUE, colour="white", family="Optima", size=textsize) +
  scale_x_continuous("Bias", limits=c(-88,88), expand=c(0,0)) +
  scale_y_continuous("Count",
                     limits=c(-1100,1100), expand=c(0,0),
                     breaks=seq(-1000,1000,250), labels=c(seq(1000,0,-250),seq(250,1000,250))) +
  ...
  theme(strip.background = element_rect(fill=panelcolor),
        ...
        axis.title = element_text(size=13, family="Optima"),
        axis.text = element_text(size=11, family="Optima"))
ggsave("img/final_poster/player_hist.png", width=5, height=4.5)
```

```{r}
sr.2 <- s2 %>%
  left_join(r2) %>%
  mutate(cost = ifelse(cost=="linear", "low linear", "high quadratic"),
         cond = paste(cost, goal),
         cond = factor(cond,
                       levels=c("low linear underestimate",
                                "high quadratic underestimate",
                                "low linear overestimate",
                                "high quadratic overestimate")))
sr.2.summ <- sr.2 %>%
  group_by(cond) %>%
  summarise(r.mean = mean(mean.kest),
            r.se = sd(mean.kest)/sqrt(n()),
...
ggplot(sr.2, aes(x=mean.ksay, y=mean.kest, colour=cond)) +
  ...
  theme(legend.position = c(0.77, 0.88),
        ...
        axis.title.x = element_markdown(),
        axis.title.y = element_markdown())
ggsave("img/final_poster/indiv_lambda.png", width=5, height=4.5)
```
