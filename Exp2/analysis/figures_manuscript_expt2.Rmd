...
r.summ <- r.data %>%
  group_by(trialLambda) %>% #instructedLambda was not told
...
r.data %>%
  ggplot(aes(x=kest.bias, y=..density.., fill=as.factor(trialLambda))) +
...
r.data %>%
  mutate(ksay.bias = ksay - k) %>%
  ggplot(aes(x=ksay.bias, y=..count.., fill=as.factor(trialLambda))) +
  geom_vline(xintercept=0) +
  geom_histogram(colour="black") +
  scale_x_continuous(limits=c(-100, 100)) +
